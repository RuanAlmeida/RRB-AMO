...
evidência verificável (nome exato de uma das 89 verificações do
...
| Transcrição: exemplo pronto + colar | Alternador "Exemplo da consulta" / "Colar transcrição"; texto colado vira a transcrição em uso, derivei trechos e zera a análise; "Usar exemplo" volta ao original; acompanha a troca de paciente | `scripts/ui/analise.js` (`estadoTranscricao`), `scripts/data/transcricao.js` | testes `transcrição oferece exemplo pronto e campo para colar`, `aplicar torna o texto colado a transcrição em uso`, `motor roda sobre o texto colado`, `voltar ao exemplo restaura a transcrição e zera a análise`, `transcrição acompanha a troca de paciente` |
...
| Dashboard em aplicação separada | O app do médico não tem dashboard (rota `#dashboard` cai em `#analise`); a gestão é uma página própria, sem dado de paciente | `gestao/index.html`, `scripts/ui/dashboard.js`, ausência de `#tela-dashboard` no `index.html` | testes `dashboard não existe mais no app do médico` (médico) e `gestão não exibe dado de paciente` (gestão) |
...
| Suíte de aceitação automatizada (médico) | Fluxo de ponta a ponta: limpo → suíte principal → persistência → **paciente2 completo** → **terceiro paciente** → volta ao anterior → reinício | `testes/smoke.html` | título `ALL PASS89 testes` |
